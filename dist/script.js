        ...
        switch (item.nodeName) {
          ...
          case 'INPUT':
            if (item.getAttribute('type') === 'checkbox') {
              state[prop] = i === 0 ? "Холодное" : "Теплое"; //i === 0 ? state[prop] ="Холодное" : state[prop] ="Теплое";
  ...
  function bindModal(triggerSelector, modalSelector, closeSelector, closeClickOverlay = true) {
    ...
    trigger.forEach(item => {
      item.addEventListener('click', e => {
        ...
        if (item.getAttribute('data-calc') === "btn-calc" && (!state.width || !state.height || state.width == 0 || state.height == 0)) {
          showMessage();
          ...
          document.body.style.overflow = "hidden"; //document.body.classList.add('modal-open'); //class from bootstrap
        }
      });
    });
    close.addEventListener('click', () => {
      ...
      document.body.style.overflow = ""; //document.body.classList.remove('modal-open'); //class from bootstrap 
    });
